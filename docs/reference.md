...
- A **fixed header** at the top shows: `received_at` (UTC) of the most recent snapshot, total match count after filter, and the active `--filter` value (or `(no filter)`).
...
- **Ctrl-C** restores the alternate-screen, prints any final summary line, and exits `0`.
...
```
<!-- Wave 2 byte-verify against Otto binary smoke per row 7 v3.2 binding — TTY ASCII art screenshot to be inserted post-implementation. -->
received_at: 2026-05-07T03:00:05Z   matches: 12   filter: google.com
┌──────────────────────┬─────────────────────────┬─────────────────────────┬─────────┬────────────────────────────────────┬──────────────────┬───────┬───────┐
│ started_at           │ source                  │ destination             │ network │ rule                               │ chains           │ up    │ down  │
├──────────────────────┼─────────────────────────┼─────────────────────────┼─────────┼────────────────────────────────────┼──────────────────┼───────┼───────┤
...
<!-- Wave 2 byte-verify against Otto binary smoke per row 7 v3.2 binding — IEC bytes formatted output + TTY in-place table screenshot pending v0.4.1 implementation. -->
...
#  see "Output (human-readable) → TTY" above for the rendered shape)
^C
...
{"type":"event","data":{"event_action":"snapshot","received_at":"2026-05-07T03:00:05Z","connections":[{"started_at":"2026-05-07T03:00:01Z","source":"192.168.1.10:55322","destination":"142.250.80.46:443","network":"tcp","rule":"DOMAIN-SUFFIX,google.com,PROXY","chains":["PROXY","JP-01"],"upload_bytes":256,"download_bytes":1024}]}}
